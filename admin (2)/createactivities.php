...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $act_desc = mysqli_real_escape_string($conn, $_POST['act_description']);
    $status = mysqli_real_escape_string($conn, $_POST['status']);

    if (isset($_FILES['act_image']) && $_FILES['act_image']['error'] === UPLOAD_ERR_OK) {
        $img_name = $_FILES['act_image']['name'];
        $img_tmp = $_FILES['act_image']['tmp_name'];
        $img_size = $_FILES['act_image']['size'];
        $img_ext = strtolower(pathinfo($img_name, PATHINFO_EXTENSION));
        $allowed_ext = ['jpg', 'jpeg', 'png', 'gif'];

        if (in_array($img_ext, $allowed_ext) && $img_size <= 2 * 1024 * 1024) {
            $new_img_name = uniqid("activity_", true) . '.' . $img_ext;
            ...
            if (move_uploaded_file($img_tmp, $upload_path)) {
                $sql = "INSERT INTO activities (act_name, act_description, act_image, status) 
                        VALUES ('$act_name', '$act_desc', '$new_img_name', '$status')";
